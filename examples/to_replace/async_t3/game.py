...
class Rules(object):
    ...
    def check(self):
        ...
        forwardslash = [0, 4 , 8]
        backslash = [2, 4, 6]
        players = [State.P1.value, State.P2.value]
    ...
    def move(self, where, who = State.P1):
        ...
        if self.map.iget(where) != State.EMPTY:
            raise Error(f"Position {self.i2c(position)} already set to {repr(self.data[position])}, must be {repr(State.EMPTY)}")
        ...
        self.map.iset(next_move, State.P2)
